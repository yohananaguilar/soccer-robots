...
//Efeitos sonoros caso erro ocorra.
...
//Set de botões, para que cada aperto emite um som!
void setup() {
...
//Aplicação sonora apenas, incrementações serão aplicadas.
...
void playSadSong(){
  // Tema de The Godfather (Speak Softly Love) - versão simplificada e triste
  
  // Parte lenta e melancólica
  tone(BUZZER_PIN, 523, 400); delay(450);  // Dó5
  tone(BUZZER_PIN, 494, 300); delay(320);  // Si4
  tone(BUZZER_PIN, 440, 500); delay(520);  // Lá4
  
  tone(BUZZER_PIN, 392, 400); delay(450);  // Sol4
  tone(BUZZER_PIN, 440, 300); delay(320);  // Lá4
  tone(BUZZER_PIN, 494, 600); delay(650);  // Si4 (bem longa e dramática)

  // Parte descendente mais triste
  tone(BUZZER_PIN, 523, 350); delay(380);  // Dó5
  tone(BUZZER_PIN, 494, 350); delay(380);  // Si4
  tone(BUZZER_PIN, 440, 450); delay(480);  // Lá4
  tone(BUZZER_PIN, 392, 600); delay(650);  // Sol4 (final grave)

  noTone(BUZZER_PIN);
  delay(300);  // pausa maior para reforçar o clima triste
}
